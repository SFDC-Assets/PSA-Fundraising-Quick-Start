...
**Last updated:** 2026-07-26 (Gift Follow-Up / Stewardship redesign shipped + ack flow renamed back to FQS_Gift_Acknowledgement for package cleanliness + CMDT field renamed FQS_Auto_Acknowledgement__c → FQS_Auto_Stewardship__c with UI copy refresh + admin/integration docs for the two-flow shape)
...
| `fqs-gift-acknowledgement-plan.md` | **superseded 2026-07-26** by `fqs-gift-followup-redesign.md` (shipped) | n/a | Original v2 conflated-flow design was implemented (`8785b00`, 2026-07-23), then split into ack + stewardship 2026-07-26 (commits `afc6007`, `ddd7386`, `a9b1ab3`). Both plans safe to archive at next cleanup pass. See `docs/gift-acknowledgement-flow.md`, `docs/gift-stewardship-flow.md`, `docs/external-tool-integration.md` for the shipped shape. |
...
| `fqs-outreach-summary-help-text-plan.md` | **shipped 2026-07-23** (commit `ad2e1f5`) | n/a — metadata only | All 9 field-meta overlays landed: AttributedAmount, DonorCount, GiftCount, OnetimeDonorCount, RecurringDonorCount, ResponseRate, TotalGiftTransactionAmount, TotalOnetimeGiftAmount, TotalRecurringGiftAmount. |
...
| `fqs-utm-platform-field-plan.md` | **shipped 2026-07-23** (commit `d6c47d8`) | n/a — metadata only | `FQS_Platform__c` picklist (19 values) added to OutreachSourceCode; `MessageChannel` + `SourceCode` inlineHelpText updated with UTM equivalents. |
...
| `fqs-duplicate-rules-plan.md` | **shipped 2026-07-25** (deploys `0AfWB00000Dbblt0AB`, `0AfWB00000Dbbyn0AB`, `0AfWB00000DbcQD0AZ`) | manual UI test pending | Ships 3 MatchingRules (Account Name+BillingCity, Account External_Id, Contact FirstName+LastName+Email) + 3 DuplicateRules (Account Organization Name+City, Account External_Id, Contact standard). Opportunity DuplicateRule dropped — platform does not support DuplicateRule on Opportunity. Person Account rides on Contact rule (Person Account inserts create a paired Contact). All Warn (allowSave=true), Warn-not-Block by design. Ten deploy learnings captured in the plan §8 for future agents — surprising XML schema quirks that all cost iteration cycles (matchingMethod not matchCriteria, actionOn+operationsOn both required, one-match-rule-per-DuplicateRule on Account, xsi:nil filter, 255-char description cap, sequential sortOrder, MatchingRules-first two-deploy pattern). |
...
| `fqs-gift-refund-plan.md` | **shipped 2026-07-23** (deploys `0AfWB00000Da4Oz`, `0AfWB00000Da4jx`, `0AfWB00000Da5GD`) | manual UI on FundFirst (full refund, partial refund, non-Paid guard, donor-launched datatable pick) | Two screen flows (`FQS_Refund_Gift`, `FQS_Refund_Gift_From_Donor`) + 2 quick actions + `GiftRefundReason` StandardValueSet extension (6 FQS values). Platform handles all cascades on `GiftRefund(Status='Completed')` insert — no downstream flow needed. Non-Paid GT pre-flight guard added (Fundraising Cloud rejects GiftRefund on non-Paid with `FIELD_INTEGRITY_EXCEPTION: Select a gift transaction that's already paid`); donor picker filters to Status=Paid + CurrentAmount>0 upfront. Donor picker uses `flowruntime:datatable` (SINGLE_SELECT) added in Flow Builder — hand-authored datatables can't emit the required `complexValue`/`ComplexObjectFieldDetails` shape at v65+ (see [[flow-datatable-source-authoring]] memory). |
