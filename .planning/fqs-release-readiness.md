...
**Last updated:** 2026-07-25 (refund flows, router refactor, donor-grouping Credit Type + Legacy giving, duplicate rules, designations D6 fields shipped)
...
- **2026-07-25 — Designations plan D6 fields shipped (Phase 6.1 + 6.2).** Added `FQS_Restriction_Release_Date__c` (Date, nullable) on both `GiftCommitment` and `GiftTransaction` — decouples time-restriction from designation per `.planning/fqs-designations-plan.md` §D6. Purpose stays on GD; time lives on the gift. Field is stored (not formula) so backfills via `sf data update` and per-installment override work. Permset `FQS_Custom_Fields` gains 2 editable entries. Deploy sequence: `0AfWB00000DbcYH0AZ` (fields) → `0AfWB00000Dbcek0AB` (permset). **Additive only — no launcher, no picklist reshape, no setup flow.** Phases 6.3+ (picklist reshape removing `With Donor Restriction - Time`, `FQS_Suggest_Designations` setup flow, launcher split UX, flexipage surfacing) stay parked. Picklist reshape still blocked by 2 live Time-restricted GDs on FundFirst (`FQS 2026 Building Fund`, `FQS 2027 Expansion Fund`) — will reclassify to Purpose during next seed refresh. Launcher work continues to be off-limits per prior gate.
