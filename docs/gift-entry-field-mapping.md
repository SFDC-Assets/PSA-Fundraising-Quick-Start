...
| `FQS_GC_Restriction_Release_Date__c`                | Date                | GiftCommitment  | `FQS_Restriction_Release_Date__c`       | Multi-year grant scenarios only. `GC`-prefixed to disambiguate from the GT-side mirror.  |
| `FQS_GC_Skip_Naming__c`                             | Checkbox            | GiftCommitment  | `FQS_Skip_Naming__c`                    | Advanced/hidden by default on the launcher. `GC`-prefixed to disambiguate from the GT-side mirror.                           |
...
| `FQS_GT_Restriction_Release_Date__c`                | Date                | GiftTransaction | `FQS_Restriction_Release_Date__c`       | Same shape as GC-side; drop into per-installment GTs. `GT`-prefixed to disambiguate from the GC-side mirror.                 |
| `FQS_GT_Skip_Naming__c`                             | Checkbox            | GiftTransaction | `FQS_Skip_Naming__c`                    | Advanced/hidden. `GT`-prefixed to disambiguate from the GC-side mirror.                                                      |
...
- **`FQS_GC_*` / `FQS_GT_*` prefix for dual-target fields.** Salesforce enforces one `sourceFieldId` → one `destinationFieldId` at the `FieldMappingConfig` level, so a single GiftEntry staging column can't fan out to both GC and GT. When a canonical FQS field lives on both objects (e.g., `FQS_Restriction_Release_Date__c`, `FQS_Skip_Naming__c`), split the GiftEntry mirror into `FQS_GC_<name>` and `FQS_GT_<name>` and map each to its side. Convention: `FQS_GC_Match_Eligible__c` was the first field established this way (2026-08-02), followed by `FQS_GC_Restriction_Release_Date__c` / `FQS_GT_Restriction_Release_Date__c` and `FQS_GC_Skip_Naming__c` / `FQS_GT_Skip_Naming__c` (2026-08-03). Same rule applies to any future FQS field that lives on both GC and GT — see [[fieldmappingconfig-metadata-type]].
