...
**Gift Processing & Data Quality**

* **Gift Transactions & Gift Commitments Guardrails** — Automates data quality, naming conventions, and fulfillment tracking for key gift objects.
* **Gift Batches & Gift Entry Templates** — Delivers high-volume processing interfaces for various gift and pledge scenarios.
* **Guided Gift Entry** — Guides users through a standardized, end-to-end flow for single gift entry.
* **Gift Refunds** — Provides a dedicated refund path that preserves historical data integrity.
* **Data Integrity & Import Guardrails** — Implements duplicate rules and idempotency keys to ensure clean bulk data imports and migrations.

**Donor Intent & Solicitation Attribution**

* **Grants, Major Gifts & Moves Management** — Manages opportunity pipelines and task ladders for grant proposals and major gift solicitations.
* **Restrictions & Designations** — Categorizes restricted revenue and automates time-based fund releases across designations.
* **Campaign Hierarchies & Templates** — Establishes multi-level campaign structures with templates to pre-populate fundraising metadata.
* **Campaign Member Lifecycle** — Syncs campaign member statuses with donor activity to ensure accurate engagement reporting.
* **Outreach Source Codes & Summaries** — Tracks channel-based attribution to link gift revenue directly to specific outreach efforts.

**Donor Recognition & Stewardship**

* **Donor Summaries & Tiers** — Calculates and categorizes donors into tiers based on annual and lifetime giving levels.
* **Acknowledgment & Stewardship Toolbox** — Provides core components to build personalized acknowledgment, stewardship, and tax receipt workflows.
* **Soft Credits & Tributes** — Enables soft credit recognition and tribute tracking without skewing revenue metrics.

**Configuration Foundation**

* **Standard Field Help Text** — Enhances standard field descriptions with operational guidance for administrators and end users.
* **Core Application Setup** — Configures the app shell, permission sets, and home page layout.

### Included Assets

An unmanaged package (link in the installation section of this document; metadata is also found in the [/force-app/main/default/](/force-app/main/default/) folder). The package ships metadata across 15 feature areas — see the [Feature Inventory](docs/fqs-feature-inventory.md) for the complete feature-by-feature breakdown, and the [Metadata Inventory](docs/fqs-metadata-inventory.md) for the full by-type index of every flow, flexipage, custom field, permission set, and supporting asset.

### Documentation Including

* This readme file
* [Feature Inventory](docs/fqs-feature-inventory.md) — all 15 feature areas with shipping scope, key flows, and related metadata.
* [Metadata Inventory](docs/fqs-metadata-inventory.md) — every metadata item organized by type and shipping status.
* [Flow Overview](docs/fqs-flow-overview.md) — all 36 shipped flows with admin-facing summaries, developer detail appendices, and Mermaid architecture diagrams for the six most complex flows.
* [Data Dictionary](docs/fqs-data-dictionary.md) — all 175 custom fields and 53 custom metadata records across 20 objects, with type, required status, and FQS-specific behavior notes.
* Salesforce Help — [Learn About Nonprofit Cloud Fundraising](https://help.salesforce.com/s/articleView?id=sfdo.fundraising_learn_about_nonprofit_cloud_fundraising.htm&type=5)
* Salesforce Help — [Nonprofit Cloud Fundraising](https://help.salesforce.com/s/articleView?id=sfdo.fundraising_nonprofit_cloud_fundraising.htm&type=5) (parent docs node)
* Salesforce Help — [Get Started with Fundraising](https://help.salesforce.com/s/articleView?id=sfdo.fundraising_get_started.htm&type=5)
* Trailhead — [Agentforce Nonprofit for Fundraising: Quick Look](https://trailhead.salesforce.com/content/learn/modules/nonprofit-cloud-for-fundraising-quick-look)
...
1. Determine where you will install the unmanaged package:
   ...
   1. **New Customer:** If you are a new customer we encourage you to install the Fundraising Quick Start accelerator into a base org available here: [https://www.salesforce.com/nonprofit/free-trial/base-trial/](https://www.salesforce.com/nonprofit/free-trial/base-trial/)
...
1. **Enable Nonprofit Cloud Fundraising and Review Fundraising Settings**

   Review the Fundraising Settings page carefully during install — several defaults are very powerful (retry counts, validation pauses) and several depend on elements that don't exist yet at pre-install time (Duplicate Rules, custom external-ID fields).
...
2. **Assign the Fundraising_Admin Permission Set Group**
...
4. **Enable Group Membership** (skip if already enabled via Stakeholder Management Quick Start)
   ...
   Group Membership enables the `AccountGroup` and `AccountGroupMember` objects that Nonprofit Cloud uses to model households — the groups that connect individual Person Account donors to their shared household record. FQS flows, Donor Gift Summary rollups, and household-level reporting all assume these objects are active. Without Group Membership enabled, household-level gift attribution and the household rollup fields on Donor Gift Summary will be empty.

   1. From **Setup**, in the **Quick Find** box, enter **Account Settings**, and then select **Account Settings**.
   2. Under **Group Membership Settings**, select **Enable Group Membership** and click **Save**.

   Supporting documentation:

   * [Enable Group Membership](https://help.salesforce.com/s/articleView?id=sfdo.npc_enable_group_membership.htm&type=5)
5. **Enable Contacts to Multiple Accounts**
...
6. **Assign the Default Workflow User**
   ...
   Choose a dedicated integration user or stable admin service account — not a personal user. If the assigned user is ever deactivated, every automation that fell back to their context will fail silently. Confirm the chosen user has an active license and, at minimum, access to Gift Transaction, Gift Commitment, Gift Designation, Gift Soft Credit, Gift Refund, Gift Tribute, and Outreach objects.

   1. From **Setup**, in the **Quick Find** box, enter **Process Automation Settings**, and then select **Process Automation Settings**.
   2. In the **Default Workflow User** field, click the lookup icon and search for your dedicated service user.
   3. Click **Save** and confirm the user's name is still displayed after the page reloads.
   4. Record the assigned user in your internal admin runbook so future admins know which account backs your automations, and add it to your user-lifecycle review so it is not accidentally deactivated during staff turnover.
...
7. **Plan Your Field History Tracking Strategy**
   ...
   FQS ships classic page layouts for **Campaign** and **Opportunity**. Field history configuration for those two objects is part of the post-install setup (§I.1). Field history on the Fundraising-managed objects (Gift Transaction, Gift Commitment, Gift Designation, etc.) is covered in Post-Install Considerations §2.
   ...
   2. **Identify the fields to track on Campaign and Opportunity**
   ...
   3. **Note the package's deployed state**
      ...
      2. **Opportunity** — the package deploys Opportunity with `enableHistory: false`. You must explicitly enable it in Setup after install (§I.1).
...
8. **Disable Campaign Influence**

   Campaign Influence must be turned off before installing the package. It is a post-install consideration to re-evaluate and re-enable once your Campaign hierarchy and attribution model are in place (see Post-Install Considerations §8).

   1. From **Setup**, in the **Quick Find** box, enter **Campaign Influence**, and then select **Campaign Influence**.
   2. Deselect **Enabled** to turn off Campaign Influence.
   3. Click **Save**.

   *Notice: Campaign Influence auto-association creates Campaign Influence records that tie campaigns to Opportunities based on Campaign Member contact matches. If it is active during install, it may attach influence records to the FQS-installed Campaign record types before your org's campaign hierarchy and attribution model are configured. Turn it back on in Post-Install Considerations §8 once your hierarchy is built and you have decided on an attribution approach.*
9. **Enable CRM Analytics for Nonprofit Cloud / Fundraising Cloud**
...
1. **Enable Field History Tracking**
   ...
   Complete the field history configuration for Campaign and Opportunity using the fields you identified in the pre-install planning step (Before You Install, step 7). Field history on the Salesforce-managed Fundraising objects (Gift Transaction, Gift Commitment, Gift Designation, etc.) is covered in Post-Install Considerations §2.
...
**V. Configure Record Access and the Fundraising Data Model**

1. **Assign Lightning Record Pages and Classic Page Layouts**
...
2. **Configure the Fundraising Data Model**
...
8. **Configure Gift Entry field mappings**
   ...
   * Developers preferring a scripted install can use the Tooling API path; see `docs/archive/fqs-fieldmappingconfig-install.md`.
...
FQS ships with **automatic placeholder Outreach Source Code creation** for every Tactical (Level 3+) campaign. One placeholder OSC named **Create First OSC** is auto-created per tactical so the record is ready and waiting for the user — they fill in **Source Code** via the Generate Source Code quick action on the OSC record page, then rename. Users are expected to add additional OSCs for each channel variant (a second OSC for social paid, a third for direct mail, etc.).
   ...
   * **During FQS Campaign Hierarchy Setup** — after the hierarchy builder inserts the Level 3 tacticals, it flips **Create First Outreach Source Code** to true on each Ask. The record-triggered flow fires on the update and creates the placeholder OSC. The Final screen shows the count.
   * **Manually per-campaign** — check the **Create First Outreach Source Code** checkbox on any Tactical campaign; the record-triggered flow creates the placeholder. Idempotent (flow Gets the placeholder by External_Id__c first and short-circuits if it already exists) — safe to re-check if the earlier OSC was deleted. The checkbox stays checked afterward as a persistent audit flag.
...
These pre-seeds are a low-friction starting point. Users edit Channel/Platform on the placeholder as needed, then click **Generate Source Code** so the Setup-configured Code Formula populates `SourceCode`, and rename off "Create First OSC" to the intended tactic label. Add additional OSCs for each channel variant on the campaign the same way.
   ...
   Beyond the FQS placeholder defaults, Salesforce Fundraising can automatically generate a standardized `SourceCode` value on each Outreach Source Code record based on a formula you define. This keeps your source codes consistent and machine-readable without relying on gift officers to type them manually.
...
*Notice: Tier Key values (`Entry`, `Mid`, `Major`) are hard-coded in the Gift Acknowledgement flow and in formula fields on Donor Gift Summary, Gift Commitment, and Gift Transaction as lookup keys. Changing them on the three seeded records will break formula evaluation. If you need to add donor tiers beyond Entry/Mid/Major, edit them directly via **Setup → Custom Metadata Types → FQS Donor Tier → Manage Records** and update the formula fields and flow logic that reference the Tier Key.*
...
2. **Review and customize the email content** — The flow uses the platform's `emailSimple` action with the shipped Classic email templates in the **FQS Templates** folder. Two templates carry the acknowledgement body:
   ...
   * `FQS_Gift_Acknowledgement_Partial` — used automatically when `TaxDeductionAmount < CurrentAmount`.
   ...
   Edit both from **Setup → Email → Classic Email Templates → FQS Templates**. Each contains a `[PROVIDE SOME INFO ABOUT YOUR HISTORY, A SPECIFIC PROGRAM, PERSON, OR YOUR TOTAL IMPACT]` placeholder that must be replaced with your organization's copy before activating the flow. To use the Lightning Email Template Builder instead of the Classic editor, assign the **FQS Email Template Builder Permission** permset (§IV.6) first.
...
FQS depends on five queues before the Gift Acknowledgement flow (§IX) is activated. None of these queues ship in the unmanaged package — Task queue metadata for the standard Task object requires org-specific member assignments that cannot be shipped as package metadata. Create all five manually. The Home Page walkthrough (§XII, accordion item 8 — *Review Automation and Queue Membership*) reinforces this — end users following the accordion top-to-bottom will only see accurate queue guidance if membership is already in place.

**Create all five queues:**

For each queue below, go to **Setup → Queues → New** and fill in the Label, Name, Supported Objects, and Queue Members as specified.


| Label                         | Name                              | Supported Objects | Notes                                                                                                                                                                   |
| ----------------------------- | --------------------------------- | ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Gift Acknowledgement Tasks`  | `FQS_Gift_Acknowledgement_Tasks`  | Task              | Required before activating §IX. Receives fallback Tasks when a donor tier is routed to Exclude All / Exclude Lifetime. Set Queue Email to your acknowledgements inbox. |
| `Executive Fundraising Tasks` | `FQS_Executive_Fundraising_Tasks` | Task, ActionPlan  | Executive-level tasks — used by Action Plans that need executive attention.                                                                                            |
| `Gift Processing Tasks`       | `FQS_Gift_Processing_Tasks`       | Task, ActionPlan  | Gift entry, acknowledgement, and tax-receipting tasks created by the Moves Management and Stewardship Action Plans.                                                     |
| `Major Donor Tasks`           | `FQS_Major_Donor_Tasks`           | Task, ActionPlan  | Research and proposal-creation tasks for the Moves Management Action Plan.                                                                                              |
| `Stewardship Tasks`           | `FQS_Stewardship_Tasks`           | Task, ActionPlan  | Cultivation tasks created by the Stewardship Action Plan.                                                                                                               |

For each queue:

1. From Setup, in the Quick Find box, enter **Queues**, and click **New**.
2. Enter the **Label** and **Queue Name** from the table above.
3. Under **Supported Objects**, add the objects listed above for that queue.
4. Under **Queue Members**, add the appropriate users, roles, or public groups for your org's development-team structure. Include at least one active user, or Tasks assigned to the queue will be invisible to work-lists.
5. Click **Save**.

See Post-Install Considerations §2 (*Review Profiles and Permission Sets*) for guidance on segregating duties across queues.

**XI. Activate Action Plan Templates**

FQS ships two Action Plan Templates — **Stewardship** and **Moves Management** — that provide structured task sequences for mid-tier donor stewardship and major gift cultivation. Both templates install as **Draft** and must be published before they can be used. Once published, a template becomes uneditable; to revise it you must create a new version.

*Notice: Publish Action Plan Templates in a Sandbox first and validate the task sequences there before publishing in production. Publishing is irreversible per version — once published, a template cannot be unpublished or edited. If you need to revise a published template, create a new version of it, which will deactivate the prior version and leave it in the org. To keep production clean, perform version iteration in Sandbox and publish only the final version to production.*

1. From the **App Launcher**, search for **Action Plans** and open the **Action Plans** app.
2. Click the **Templates** tab.
3. Locate the **Stewardship** template and click its name to open it.
4. Review the task steps, due-date offsets, and queue assignments. Confirm each step's assigned queue or user matches the queues you created in §X.
5. Click **Publish**. The template status changes to **Active**.
6. Repeat for the **Moves Management** template.

Supporting documentation:

* [Create and Manage Action Plan Templates](https://help.salesforce.com/s/articleView?id=ind.fsc_action_plans_create_templates.htm&type=5)
...
* **6. Guidance on Acknowledgement, Stewardship, and Tax Receipting** — the FQS date model (Transaction Date vs. Donor Tax Date vs. Acknowledgement Date vs. Tax Receipt Date) and the two-flow acknowledgement + stewardship pattern.
...
* **Fundraising Prerequisites:** [Nonprofit Cloud Fundraising Prerequisites](https://help.salesforce.com/s/articleView?id=sfdo.fundraising_prerequisites.htm&type=5)
* **Editions & Permissions:** [Nonprofit Cloud Fundraising Editions and Permissions](https://help.salesforce.com/s/articleView?id=sfdo.fundraising_editions_and_permissions.htm&type=5)
* **Gift Entry & Batches:** [Gift Entry Processing](https://help.salesforce.com/s/articleView?id=sfdo.fundraising_gift_entry_processing.htm&type=5)
* **Recurring Donations:** [Gift Commitments](https://help.salesforce.com/s/articleView?id=sfdo.fundraising_gift_commitments.htm&type=5) *(Recurring Donations are modeled as Gift Commitments in Nonprofit Cloud Fundraising.)*
* **Designations:** [Manage Designations](https://help.salesforce.com/s/articleView?id=sfdo.fundraising_manage_designations.htm&type=5)
* **Reporting & Analytics:** [Fundraising Intelligence](https://help.salesforce.com/s/articleView?id=sfdo.fundraising_fundraising_intelligence.htm&type=5)
* **Trailhead Trail:** [Manage Fundraising with Agentforce Nonprofit](https://trailhead.salesforce.com/content/learn/trails/manage-fundraising-with-nonprofit-cloud)
* **Trailhead Module:** [Agentforce Nonprofit for Fundraising: Quick Look](https://trailhead.salesforce.com/content/learn/modules/nonprofit-cloud-for-fundraising-quick-look)
...
* **Standard Rules:** [Standard Duplicate Rules Reference](https://help.salesforce.com/s/articleView?id=sales.duplicate_rules_standard_rules.htm&type=5)
...
FQS is authored against a single-currency org. Multi-currency is supported by the underlying Nonprofit Cloud objects, but enabling it is an org-wide, irreversible decision with significant consequences for existing data and reports.

*Notice: Once multi-currency is enabled it cannot be disabled without Salesforce Support involvement. Evaluate this decision carefully with your finance team before enabling. See [Considerations for Enabling Multiple Currencies](https://help.salesforce.com/s/articleView?id=sales.admin_enable_multicurrency_implications.htm&type=5) before proceeding.*

If your org requires multi-currency, complete the following before going live with FQS:

1. From **Setup**, in the **Quick Find** box, enter **Currencies**, and then select **Manage Currencies**.
2. Click **Enable**, confirm the impact warning, and select your org's **Corporate Currency** (the base currency all records are stored in).
3. Add each additional active currency your org transacts in using **New** in the Active Currencies list. Set exchange rates.
4. Confirm that users entering gifts in non-corporate currencies will see amounts converted at the corporate rate in rollup fields — this is expected behavior, not a bug.

**FQS-specific impacts when multi-currency is enabled:**

* **FQS reports and dashboard** — the seven shipped reports and the FQS Donor Tiers dashboard aggregate `SUM(CurrentAmount)`, `SUM(OriginalAmount)`, and related rollup fields in each record's transaction currency. No cross-currency conversion is applied at the report level. Donors who give in different currencies will show inflated or deflated totals depending on which currency is selected as the report's display currency. Audit each report after enabling multi-currency and add currency filters if needed.
* **Donor Gift Summary rollups** — the Nonprofit Cloud DPE rollups that populate `GiftsThisYearAmount`, `TotalGiftsAmount`, and the FQS formula fields (`FQS_Is_Major_Annual_Donor__c`, etc.) aggregate in the corporate currency using the exchange rate at the time the DPE runs. Rates are not retroactively applied to historical records; a rate change between DPE runs can shift a donor's tier classification without any actual gift activity.
* **Donor Tier thresholds** — the `FQS_Donor_Tier__mdt` threshold fields (`One_Time_Min_Amount__c`, `Annual_Min_Amount__c`, `Lifetime_Min_Amount__c`) are plain number fields with no currency awareness. Thresholds are interpreted in the corporate currency. Organizations transacting heavily in non-corporate currencies should set thresholds in the corporate-currency equivalent.

**Multi-entity**

FQS is authored against a single-entity org. If your org uses multiple business units with data-sharing rules, re-scope the FQS Donor Tiers dashboard's Dynamic Dashboard `runningUser` per entity, and consider cloning the Custom Report Types (`fqs_*_Deluxe`) with entity-scoped filters before rolling out to end users.
...
* [Considerations for Enabling Multiple Currencies](https://help.salesforce.com/s/articleView?id=sales.admin_enable_multicurrency_implications.htm&type=5)
* [Manage Multiple Currencies](https://help.salesforce.com/s/articleView?id=platform.admin_currency.htm&type=5)
* [Advanced Currency Management](https://help.salesforce.com/s/articleView?id=platform.administration_currency_dated_exchange_rates.htm&type=5)
...
* Amy Bucciferro (Arkus)
* Erin Ramirez (Arkus)
* Judi Sohn (Elevation Solutions)
* Ethan Norris (Elevation Solutions)
* Tobias Ward (Galvin Tech)
* Nate Rush (Idealist)
* Kathy Dune (Idealist)
* Dar Veverka
